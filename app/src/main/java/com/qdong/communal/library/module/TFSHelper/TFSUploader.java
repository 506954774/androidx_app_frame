...
public class TFSUploader {
    ...
    private LinkLinkApi mApiService;//api
     ...
     * @author Chuck
     **/
    public Observable<LinkLinkNetInfo>  executeUploadMultipleFiles(ArrayList<String> filePaths){
     ...
     * @author Chuck
     **/
    public Observable<LinkLinkNetInfo>  executeSingleFile(String filePath){
        if(TextUtils.isEmpty(filePath)){
            ...
            if(file.exists()){
                ...
                return mApiService.uploadSingleFile(body);
            }
            return null;
        }
    }
  ...
  /*  *//**
    ...
    public Subscription  uploadMultipleFilesThenUpdateUrl(
            ArrayList<String> filePaths,
            HashMap<String, String> loginMap,
            Func1<LinkLinkNetInfo, Observable<LinkLinkNetInfo>> flatMap,
     ...
     * @author Chuck
     **/
    public Subscription  uploadSingleFileThenUpdateUrl(
            String filePaths,
            HashMap<String, String> loginMap,
            Func1<LinkLinkNetInfo, Observable<LinkLinkNetInfo>> flatMap,
            Observer<LinkLinkNetInfo> observer){
        ...
        if(TextUtils.isEmpty(filePaths)||flatMap==null||observer==null){
            return null;
        }
        else {
            Subscription subscription=
                    executeSingleFile(filePaths)//tfs图片上传
                            .flatMap(flatMap)//获取返回的路径,调用某个接口
                            .subscribeOn(Schedulers.io())//指定被观察者的执行线程
                            .observeOn(AndroidSchedulers.mainThread())//切换到主线程,因为如果正常反回了,我要将事件直接交给观察者,观察者通常在主线程
                            .flatMap(RxHelper.getInstance(mContext).judgeSessionExpired(observer))//判断session是否过期,如果过期了,会抛出异常到事件流里
     ...
     * @author Chuck
     **/
    public Subscription uploadMultipleFiles(ArrayList<String> filePaths,HashMap<String, String> map,Observer<LinkLinkNetInfo> observer) {
