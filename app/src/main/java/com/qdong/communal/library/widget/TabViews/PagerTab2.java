...
/**
 * PagerTab2
 * 配合viewpager使用的横向tab,与viewpager关联
 * 通过{@link #setViewPager(ViewPager)}实现与viewPager关联,此viewPager的adpater要重写{@link androidx.core.view.PagerAdapter#getPageTitle(int)}来给tab提供每个栏目的标题
 ...
 **/
public class PagerTab2 extends ViewGroup {
    ...
    private TextView[] mTextViews;//为了动态修改每个按钮的颜色,把按钮存入这个数组里

    private boolean smoothScroll = true;     //控制ViewPager切换时是否有滑动效果

    public ClickTabsListener click;

    public PagerTab2(Context context) {
        this(context, null);
    }

    public PagerTab2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PagerTab2(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(TypedArray mTypedArray) {
        ...
        //tab的字体颜色
        mTabTextColorResId = mTypedArray.getResourceId(R.styleable.PagerTab_tabTextColorResId, R.drawable.bd_pager_tab_text_selector);
        //tab文字是否加粗
    ...
    public void renovatePagerTitle() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (childAt instanceof TextView) {
                ((TextView) childAt).setText(mViewPager.getAdapter().getPageTitle(i));
            }
        }
    }
    ...
    private void addTextTab(final int position, CharSequence title) {
        ...
        float textWidth = tab.getPaint().measureText(title.toString());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        LogUtil.e("addTextTab", "onMeasure(int widthMeasureSpec, int heightMeasureSpec)");
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int offset = (mIndicatorWidth - newWith) / 2;
        ...
        RectF oval3 = new RectF(mIndicatorLeft + mTabPaddingLeftRight, height - mIndicatorHeight,
                mTabPaddingLeftRight + mIndicatorLeft + newWith, height);// 设置个新的长方形
    ...
    public interface ClickTabsListener{
        void getPosition(int index);
    }
    public void setClickTabsListener(ClickTabsListener click){
        this.click = click;
    }
}
