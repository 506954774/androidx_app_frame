...
import com.qdong.communal.library.widget.custommask.ReloadCallback;
...
public abstract class BaseRefreshableListFragment<T> extends Fragment {
    ...
    private void initView(View view) {
        ...
        //去掉阴影
        mRefreshRecyclerView.getmRecyclerView().setOverScrollMode(View.OVER_SCROLL_NEVER);
                        ...
                        if (info.isSuccess()) {
                            if (info.getData() != null) {
                                try {
                                    ...
                                    List<T> list = resolveData(info.getData());
                        ...
                        if (info.isSuccess()) {
                            if (info.getData() != null) {
                                try {
                                    ...
                                    if (list==null||list.size()==0) {//如果刷新成功后没有数据了,则显示没有内容视图
                                        loadingView.showNoContent();
                                    }
                                    mAdapter.setData(list);
                                    mRefreshRecyclerView.onRefreshCompleted();
                            ...
                            if (info.getData() != null) {
                                try {
                                    ...
                                    if (list == null || list.size() == 0) {
                                        ...
                                        mAdapter.getAllData().addAll(list);
    ...
    protected void loadingMoreData() {
        ...
        if (mAutoCancelLoadMore) {//数据不足pageSize时是否屏蔽上拉事件
            if (mAdapter.getAllData().size() < mPageSize) {
                mRefreshRecyclerView.onNoMoreData();
                return;
            }
        }
