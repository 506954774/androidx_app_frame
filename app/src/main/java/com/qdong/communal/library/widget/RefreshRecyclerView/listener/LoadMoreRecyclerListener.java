...
import com.qdong.communal.library.widget.RefreshRecyclerView.widget.RefreshLoadingLayout;
import com.qdong.communal.library.widget.RefreshRecyclerView.widget.RotateLoadingLayout;
...
public class LoadMoreRecyclerListener extends RecyclerView.OnScrollListener {
     ...
     * 2016/06/21 by:Chuck
     */
    public  synchronized void onNoMoreData(RecyclerView recyclerView){
        isLoading = true;
        if (null == mFooterLoadingLayout) {
            mFooterLoadingLayout = new RotateLoadingLayout(mContext, RecyclerMode.BOTTOM);
        }
        ...
        ((RotateLoadingLayout)mFooterLoadingLayout).noMoreData();
    }
    /****
     * 加上这部分代码,实现"无更多数据"的footview展示
