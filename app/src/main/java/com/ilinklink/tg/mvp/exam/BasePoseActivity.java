...
import com.ilinklink.tg.mvp.fuwocheng.FuwochengActivity;
...
public  class BasePoseActivity extends BaseMvpActivity<ActivityFuwochengBinding> implements View.OnClickListener  ,OnRequestPermissionsResultCallback, OnItemSelectedListener, CompoundButton.OnCheckedChangeListener, ExamContract
        .ExamView {
    ...
    private void resetSubject() {
        mExamUUID = getIntent().getStringExtra(EXAM_UUID);
        if (TextUtils.isEmpty(mExamUUID)) {
            ToastUtils.showShort("考试数据不存在,请联系管理员");
            finish();
            return;
        } else {
            mExam = DBHelper.getInstance(this).getExamInfo(mExamUUID);
            if (mExam == null) {
                ToastUtils.showShort("考试数据不存在,请联系管理员");
                finish();
                return;
            ...
            public void call(Long aLong) {
                try {
                    ...
                    if (aLong >= 0 && aLong <= mTimer) {
                        ...
                        /**取消此任务**/
                        RxHelper.getInstance(BasePoseActivity.this).unsubscribe(mSubscriptionCount);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
