...
public  class ExamActivity extends BasePoseActivity{
    ...
    protected void startExam(){
        ...
        if(mExam!=null){
            ...
            if(mExam.getLimitTime()!=null){
                if(mExam.getLimitTime().compareTo(1000L)>0){
                    mTimer=mExam.getLimitTime()/1000;
                    countDown();
                }
            }
        }
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_commit_exam_result:
                if(mStudentExam!=null){
                    ...
                    if(mIsCountDown){
                        ToastUtil.showCustomMessageShort(this,getString(R.string.exam_result_counting));
                        return;
                    }

                    mIsExamming=false;
                    mLoadingView.showLoading(getString(R.string.exam_result_upload));
                    HashMap<String, Object> map=new HashMap<>();
                    map.put("examResultId",mStudentExam.getResultUUID());
                    int result=0;
                    if(mActionCountCallBackBean!=null){
                        result=mActionCountCallBackBean.getGooCount();
                    }
                    map.put("result",result);


                    SdCardLogUtil.logInSdCard("ExamActivity","ExamActivity,提交考试成绩,参数:"+map);

                    mPresenter.uploadExamResult(map);
                }
                else {
                    go2FaceRecognize();
                }
