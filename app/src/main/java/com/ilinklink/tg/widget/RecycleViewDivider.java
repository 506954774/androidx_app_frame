...
public class RecycleViewDivider extends RecyclerView.ItemDecoration {
    ...
    public RecycleViewDivider(Context context,int dividerWidth) {
        this(context, false);
        this.mDividerWidth = dividerWidth;
    }

    public RecycleViewDivider(Context context, boolean hasBorder) {
    ...
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view,
        ...
        int itemDividerWidth = (spanCount + (hasBorder ? 1 : -1)) * mDividerWidth / spanCount;
        int left;
        int right;
        if (hasBorder) {
            //有边框
            // left = ( 行中位置 + 1 ) * 分割线宽度 - 行中位置 * 每个item内分割线占用的宽度
            left = (indexHorizontal + 1) * mDividerWidth - indexHorizontal * itemDividerWidth;
            //right = 每个item内分割线占用的宽度 - left
            right = itemDividerWidth - left;
        } else {
            //无边框
            //left = 行中位置 * ( 分割线宽度 - 每个item内分割线占用的宽度 )
            left = indexHorizontal * (mDividerWidth - itemDividerWidth);
            //right = 每个item内分割线占用的宽度 - left
            right = itemDividerWidth - left;
        }
        ...
        //上下偏移
        //横向分割线高度
        int dividerHeight = mDividerWidth;
